...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        # Compute gradients
        output_layer_error = (activated_output - y) * output_layer_activation_derivative
        hidden_layer_error = np.multiply((np.transpose(self.hidden_to_output_weights))) # (3 by 1 matrix)

        bias_gradients = hidden_layer_error
        hidden_to_output_weight_gradients = np.transpose(hidden_layer_activation)
        input_to_hidden_weight_gradients = np.transpose(input_values*np.transpose(hidden_to_output_weight_gradients))
